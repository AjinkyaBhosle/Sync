...
export default function Generate() {
  ...
  const [extendSongId, setExtendSongId] = useState<string | null>(null);
  const [extendPrompt, setExtendPrompt] = useState('');
  const [showAddVocalsModal, setShowAddVocalsModal] = useState(false);
  const [addVocalsSongId, setAddVocalsSongId] = useState<string | null>(null);
  const [addVocalsPrompt, setAddVocalsPrompt] = useState('');
  const [showAddInstrumentalModal, setShowAddInstrumentalModal] = useState(false);
  const [addInstrumentalSongId, setAddInstrumentalSongId] = useState<string | null>(null);
  const [addInstrumentalPrompt, setAddInstrumentalPrompt] = useState('');
  const [showTimestampedLyricsModal, setShowTimestampedLyricsModal] = useState(false);
  const [timestampedLyrics, setTimestampedLyrics] = useState('');
  const [showSeparateVocalsModal, setShowSeparateVocalsModal] = useState(false);
  ...
  const createMusicVideo = async (taskId: string) => {
    try {
      const response = await fetch('/api/create-music-video', {
        method: 'POST',
      ...
      if (data.error) {
        ...
        setShowCreateMusicVideoModal(true);
      }
    } catch {
      setError('Failed to create music video. Please try again.');
    }
  };


  const downloadWav = async (taskId: string) => {
    try {
      ...
      if (data.error) {
        ...
        window.open(data.wavUrl, '_blank');
      }
    } catch {
      setError('Failed to download WAV. Please try again.');
    }
  };


  const separateVocals = async (taskId: string) => {
    try {
      ...
      if (data.error) {
        ...
        setVocalUrl(data.vocalUrl);
        setInstrumentalUrl(data.instrumentalUrl);
        setShowSeparateVocalsModal(true);
      }
    } catch {
      setError('Failed to separate vocals. Please try again.');
    }
  };


  const getTimestampedLyrics = async (taskId: string) => {
    try {
      const response = await fetch('/api/timestamped-lyrics', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ taskId }),
      });
      ...
      if (data.error) {
        ...
        setShowTimestampedLyricsModal(true);
      }
    } catch {
      setError('Failed to get timestamped lyrics. Please try again.');
    }
  };


  const addInstrumental = async () => {
    if (!addInstrumentalSongId || !addInstrumentalPrompt.trim()) {
      setError('Please enter a prompt for the instrumental.');
      return;
    }
    ...
    setAudioUrl('');
    setSongTitle('');
    setStatusMessage('Adding instrumental...');
    setShowAddInstrumentalModal(false);

    try {
      const response = await fetch('/api/add-instrumental', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ taskId: addInstrumentalSongId, prompt: addInstrumentalPrompt }),
      });
      ...
      setError('Failed to add instrumental. Please try again.');
      setLoading(false);
    }
  };
  ...
  const addVocals = async () => {
    if (!addVocalsSongId || !addVocalsPrompt.trim()) {
      setError('Please enter a prompt for the vocals.');
      return;
    }
    ...
    setAudioUrl('');
    setSongTitle('');
    setStatusMessage('Adding vocals...');
    setShowAddVocalsModal(false);

    try {
      const response = await fetch('/api/add-vocals', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ taskId: addVocalsSongId, prompt: addVocalsPrompt }),
      });
      ...
      setError('Failed to add vocals. Please try again.');
      setLoading(false);
    }
  };


  const extendSong = async () => {
    if (!extendSongId || !extendPrompt.trim()) {
    ...
    setAudioUrl('');
    setSongTitle('');
    setStatusMessage('Starting extension...');
    ...
    try {
      const response = await fetch('/api/extend', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ taskId: extendSongId, prompt: extendPrompt }),
      });
      ...
      setError('Failed to extend song. Please try again.');
      setLoading(false);
    }
  };
      ...
      <main className="p-4 md:p-8">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-card-background p-6 rounded-2xl shadow-lg">
            ...
            {showCreateMusicVideoModal && (
              <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div className="bg-card-background p-6 rounded-2xl shadow-lg max-w-lg w-full">
                  <h3 className="text-lg font-semibold mb-2">Music Video</h3>
                  <video controls className="w-full" src={musicVideoUrl}>
                    Your browser does not support the video tag.
                  </video>
                  <button
                    onClick={() => setShowCreateMusicVideoModal(false)}
                    className="mt-4 w-full bg-gray-600 text-white py-2 rounded-lg font-semibold hover:opacity-90 transition"
                  >
                    Close
                  </button>
                </div>
            ...
            {showSeparateVocalsModal && (
              <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div className="bg-card-background p-6 rounded-2xl shadow-lg max-w-lg w-full">
                  <h3 className="text-lg font-semibold mb-2">Separated Tracks</h3>
                  <div className="flex flex-col gap-4 mt-4">
                    <a href={vocalUrl} target="_blank" rel="noreferrer" className="w-full bg-primary-accent text-white py-2 rounded-lg font-semibold hover:opacity-90 transition text-center">Download Vocals</a>
                    <a href={instrumentalUrl} target="_blank" rel="noreferrer" className="w-full bg-primary-accent text-white py-2 rounded-lg font-semibold hover:opacity-90 transition text-center">Download Instrumental</a>
                  </div>
                  <button
                    onClick={() => setShowSeparateVocalsModal(false)}
                    className="mt-4 w-full bg-gray-600 text-white py-2 rounded-lg font-semibold hover:opacity-90 transition"
                  >
                    Close
                  </button>
            ...
            {showTimestampedLyricsModal && (
              <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div className="bg-card-background p-6 rounded-2xl shadow-lg max-w-lg w-full">
                  <h3 className="text-lg font-semibold mb-2">Timestamped Lyrics</h3>
                  <p className="text-gray-300 whitespace-pre-line max-h-96 overflow-y-auto">{timestampedLyrics}</p>
                  <button
                    onClick={() => setShowTimestampedLyricsModal(false)}
            ...
            {showAddInstrumentalModal && (
              <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div className="bg-card-background p-6 rounded-2xl shadow-lg max-w-lg w-full">
                  <h3 className="text-lg font-semibold mb-2">Add Instrumental</h3>
                  <textarea
                    value={addInstrumentalPrompt}
                    onChange={(e) => setAddInstrumentalPrompt(e.target.value)}
                    placeholder="E.g., a piano melody, a heavy metal riff..."
                    className="w-full h-36 p-4 bg-input-background border-2 border-gray-700 rounded-lg focus:border-primary-accent focus:outline-none resize-none transition"
                  />
                  <div className="flex gap-4 mt-4">
                    <button
                      onClick={() => setShowAddInstrumentalModal(false)}
                      className="w-full bg-gray-600 text-white py-2 rounded-lg font-semibold hover:opacity-90 transition"
                    >
                      Cancel
                    </button>
                    <button
                      onClick={addInstrumental}
                      className="w-full bg-primary-accent text-white py-2 rounded-lg font-semibold hover:opacity-90 transition"
                    >
                      Add Instrumental
                    </button>
                  </div>
            ...
            {showAddVocalsModal && (
              <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div className="bg-card-background p-6 rounded-2xl shadow-lg max-w-lg w-full">
                  <h3 className="text-lg font-semibold mb-2">Add Vocals</h3>
                  <textarea
                    value={addVocalsPrompt}
                    onChange={(e) => setAddVocalsPrompt(e.target.value)}
                    placeholder="E.g., a female voice singing about the rain..."
                    className="w-full h-36 p-4 bg-input-background border-2 border-gray-700 rounded-lg focus:border-primary-accent focus:outline-none resize-none transition"
                  />
                  <div className="flex gap-4 mt-4">
                    <button
                      onClick={() => setShowAddVocalsModal(false)}
                      className="w-full bg-gray-600 text-white py-2 rounded-lg font-semibold hover:opacity-90 transition"
                    >
                      Cancel
                    </button>
            ...
            {showExtendModal && (
              <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div className="bg-card-background p-6 rounded-2xl shadow-lg max-w-lg w-full">
                  <h3 className="text-lg font-semibold mb-2">Extend Song</h3>
                  <textarea
                    value={extendPrompt}
                    onChange={(e) => setExtendPrompt(e.target.value)}
                    placeholder="E.g., add a guitar solo, change the mood to be more melancholic..."
                    className="w-full h-36 p-4 bg-input-background border-2 border-gray-700 rounded-lg focus:border-primary-accent focus:outline-none resize-none transition"
                  />
                  <div className="flex gap-4 mt-4">
                    <button
                    ...
                    <div key={song.id} className="p-4 bg-input-background rounded-lg flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="px-4 py-2 bg-primary-accent text-white text-xs font-semibold rounded-full hover:bg-secondary-accent transition cursor-pointer"
                        >
                          Play
                        </button>
                        <div className="relative">
                          ...
                          {openMenuId === song.id && (
                            <div className="absolute right-0 mt-2 w-48 bg-card-background border border-gray-700 rounded-lg shadow-lg z-10">
                              <a href="#" onClick={() => { setExtendSongId(song.id); setShowExtendModal(true); setOpenMenuId(null); }} className="block px-4 py-2 text-sm text-gray-300 hover:bg-gray-700">Extend Music</a>
                              <a href="#" onClick={() => { setAddVocalsSongId(song.id); setShowAddVocalsModal(true); setOpenMenuId(null); }} className="block px-4 py-2 text-sm text-gray-300 hover:bg-gray-700">Add Vocals</a>
                              <a href="#" onClick={() => { setAddInstrumentalSongId(song.id); setShowAddInstrumentalModal(true); setOpenMenuId(null); }} className="block px-4 py-2 text-sm text-gray-300 hover:bg-gray-700">Add Instrumental</a>
                              <a href="#" onClick={() => { getTimestampedLyrics(song.id); setOpenMenuId(null); }} className="block px-4 py-2 text-sm text-gray-300 hover:bg-gray-700">Get Timestamped Lyrics</a>
                              <a href="#" onClick={() => { separateVocals(song.id); setOpenMenuId(null); }} className="block px-4 py-2 text-sm text-gray-300 hover:bg-gray-700">Separate Vocals</a>
                              <a href="#" onClick={() => { downloadWav(song.id); setOpenMenuId(null); }} className="block px-4 py-2 text-sm text-gray-300 hover:bg-gray-700">Download as WAV</a>
                              <a href="#" onClick={() => { createMusicVideo(song.id); setOpenMenuId(null); }} className="block px-4 py-2 text-sm text-gray-300 hover:bg-gray-700">Create Music Video</a>
                            </div>
                          )}
                        </div>
                      </div>
