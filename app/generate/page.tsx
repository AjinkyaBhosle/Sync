...
• Focusing on musical style and mood instead of specific topics');
  ...
  const generateSong = async () => {
    ...
    const sensitivePatterns = /(kill|death|violence|explicit|nsfw|sexual|drug|hate)/i;
...
<main className="min-h-screen bg-black flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-10 left-10 text-6xl opacity-5 animate-float text-green-500">🎵</div>
        <div className="absolute top-20 right-20 text-4xl opacity-5 animate-float-delayed text-teal-500">🎶</div>
        <div className="absolute bottom-40 left-20 text-5xl opacity-5 animate-float-slow text-purple-500">🎼</div>
        <div className="absolute bottom-20 right-32 text-7xl opacity-5 animate-float text-pink-500">🎹</div>
        <div className="absolute top-1/3 left-1/4 text-3xl opacity-5 animate-float-delayed text-indigo-500">🎸</div>
        <div className="absolute top-2/3 right-1/4 text-4xl opacity-5 animate-float-slow text-red-500">🎤</div>
        <div className="absolute bottom-1/2 left-10 text-5xl opacity-5 animate-float text-blue-500">🎺</div>
        <div className="absolute top-1/2 right-10 text-6xl opacity-5 animate-float-delayed text-yellow-500">🎧</div>
      </div>

      <div className="max-w-2xl w-full bg-gray-900 rounded-2xl shadow-2xl p-8 relative z-10">
        ...
        <div className="mb-4">
          <button
            ...
            className="w-full text-xs text-green-400 hover:text-green-300 transition flex items-center justify-center gap-2"
          >
            <span>💡</span>
          ...
          {showTips && (
            <div className="mt-3 p-4 bg-gray-800 rounded-lg text-left space-y-3 border border-gray-700">
              <div>
                <h3 className="text-sm font-bold text-white mb-2">✅ Best Practices:</h3>
              ...
              <div>
                <h3 className="text-sm font-bold text-white mb-2">❌ Avoid:</h3>
                <ul className="text-xs text-gray-400 space-y-1 ml-4">
              ...
              <div>
                <h3 className="text-sm font-bold text-white mb-2">🎵 Example Prompts:</h3>
        ...
        {showSongs && (
          <div className="mb-6 p-4 bg-gray-800 rounded-lg max-h-96 overflow-y-auto">
            <h3 className="font-bold mb-3 text-white">Your Songs ({songs.length})</h3>
            {songs.length === 0 ? (
              <p className="text-gray-400 text-sm">No songs yet. Generate your first one!</p>
            ) : (
              <div className="space-y-2">
                {songs.map((song: Song) => (
                  <div key={song.id} className="p-3 bg-gray-700 rounded border border-gray-600 flex items-center justify-between">
...
setShowSongs(false);
                      ...
                      className="ml-2 px-3 py-1 bg-green-500 text-white text-xs rounded hover:bg-green-600"
                    >
                      ▶️ Play
                    </button>
                  </div>
        ...
        <div className="space-y-4">
          <div className="relative">
            <textarea
              ...
              className="w-full h-32 p-4 bg-gray-800 border-2 border-gray-700 rounded-lg text-white focus:border-green-500 focus:outline-none resize-none"
              disabled={loading}
              maxLength={500}
            />
          ...
          <button
            ...
            disabled={loading}
            className="w-full bg-gradient-to-r from-green-500 to-teal-500 text-white py-4 rounded-lg font-semibold hover:from-green-600 hover:to-teal-600 transition disabled:opacity-50 disabled:cursor-not-allowed"
          >
        ...
        <div className="mt-6 pt-4 border-t border-gray-700 text-center">
          <p className="text-xs text-gray-500">
            Created with ❤️ by <span className="font-semibold text-green-400">Ajinkya Bhosle</span>
          </p>
        </div>
      </div>
    </main>
